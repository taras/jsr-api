...
import { PackageListParams, PackageListResponse, Packages } from './resources/packages';
import { PublishingTask, PublishingTasks } from './resources/publishing-tasks';
...
import {
  Authorization,
  AuthorizationExchangeParams,
  AuthorizationExchangeResponse,
  AuthorizationStartParams,
  Authorizations,
  CreateAuthorizationResponse,
} from './resources/authorizations/authorizations';
import { ScopeCreateParams, ScopeUpdateParams, Scopes } from './resources/scopes/scopes';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['JSR_API_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['JSR_API_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Jsr API API.
 */
export class JsrAPI {
  bearerToken: string;
   ...
   * @param {string} [opts.baseURL=process.env['JSR_API_BASE_URL'] ?? https://api.jsr.io] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('JSR_API_BASE_URL'),
    bearerToken = readEnv('BEARER_TOKEN'),
    ...opts
  }: ClientOptions = {}) {
    if (bearerToken === undefined) {
      throw new Errors.JsrAPIError(
        "The BEARER_TOKEN environment variable is missing or empty; either provide it, or instantiate the JsrAPI client with an bearerToken option, like new JsrAPI({ bearerToken: 'My Bearer Token' }).",
      );
    }
    ...
    const options: ClientOptions = {
      bearerToken,
      ...opts,
      baseURL: baseURL || `https://api.jsr.io`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? JsrAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('JSR_API_LOG'), "process.env['JSR_API_LOG']", this) ??
      defaultLogLevel;
  ...
  static JsrAPIError = Errors.JsrAPIError;
  ...
  publishingTasks: API.PublishingTasks = new API.PublishingTasks(this);
  stats: API.Stats = new API.Stats(this);
}
JsrAPI.Scopes = Scopes;
JsrAPI.Packages = Packages;
JsrAPI.User = User;
JsrAPI.Users = Users;
JsrAPI.Authorizations = Authorizations;
JsrAPI.PublishingTasks = PublishingTasks;
export declare namespace JsrAPI {
  ...
  export { PublishingTasks as PublishingTasks, type PublishingTask as PublishingTask };

  export { type Stats as Stats };

  export type PackageVersion = API.PackageVersion;
  export type Scope = API.Scope;
  export type ScopeInvite = API.ScopeInvite;
  export type ScopeMember = API.ScopeMember;
