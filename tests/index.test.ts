...
import { APIPromise } from 'jsr-api/core/api-promise';

import util from 'node:util';
import JsrAPI from 'jsr-api';
import { APIUserAbortError } from 'jsr-api';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['JSR_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: JsrAPI) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new JsrAPI({ logger: logger, logLevel: 'debug', bearerToken: 'My Bearer Token' });
    ...
    test('default logLevel is warn', async () => {
      const client = new JsrAPI({ bearerToken: 'My Bearer Token' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new JsrAPI({ logger: logger, logLevel: 'info', bearerToken: 'My Bearer Token' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['JSR_API_LOG'] = 'debug';
      const client = new JsrAPI({ logger: logger, bearerToken: 'My Bearer Token' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['JSR_API_LOG'] = 'not a log level';
      const client = new JsrAPI({ logger: logger, bearerToken: 'My Bearer Token' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'JSR_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['JSR_API_LOG'] = 'debug';
      const client = new JsrAPI({ logger: logger, logLevel: 'off', bearerToken: 'My Bearer Token' });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['JSR_API_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new JsrAPI({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['JSR_API_BASE_URL'] = 'https://example.com/from_env';
      const client = new JsrAPI({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['JSR_API_BASE_URL'] = ''; // empty
      const client = new JsrAPI({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://api.jsr.io');
    });

    test('blank env variable', () => {
      process.env['JSR_API_BASE_URL'] = '  '; // blank
      const client = new JsrAPI({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://api.jsr.io');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new JsrAPI({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new JsrAPI({ bearerToken: 'My Bearer Token' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new JsrAPI({ bearerToken: 'My Bearer Token', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new JsrAPI({ bearerToken: 'My Bearer Token', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new JsrAPI({ bearerToken: 'My Bearer Token', fetch: testFetch });
